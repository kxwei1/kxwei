...
export default {
  ...
  mounted() {
    ...
    if (this.$route.params.roleid) {
      this.tip = "修改";
      this.$http.get(this.$apis.roleinfo,{ id: this.$route.params.roleid }
      ).then(res => {
        this.info = res.list;
    ...
    this.$http.get(this.$apis.menulist,{ istree: 1 }
    ).then(res => {
      this.menus = res.list;
    });
  },
    ...
    submitInfo(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.$route.params.roleid) {
            (url = this.$apis.roleedit), (data.id = this.$route.params.roleid);
          }
